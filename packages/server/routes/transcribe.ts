...
import { TranscriptionResponse, ApiResponse } from '@listener/shared';
...
// Custom error interface for transcription errors
...
router.get('/', async (req: Request, res: Response): Promise<void> => {
    ...
    const spotifyUrl: string = req.query.url as string;
    ...
    try {
        ...
        const slug: string = await podcastService.getPodcastSlug(spotifyUrl);
        const feedUrl: string = await podcastService.getPodcastFeed(slug);
        const rssText: string = await podcastService.fetchRssFeed(feedUrl);
        const rssData = podcastService.parseRssFeed(rssText);
        const mp3Url: string = podcastService.extractMp3Url(rssData);

        // Fetch MP3 file as a stream
        const audioRes: globalThis.Response = await fetch(mp3Url);
        if (!audioRes.ok) {
        ...
        const transcriptText: string = await transcribe(tmpFile);
...
router.post('/', async (req: Request, res: Response): Promise<void> => {
    ...
    const { spotifyUrl }: TranscribePostRequest = req.body;
    ...
    let tmpFile: string | undefined;
    let slug: string | undefined; // Define slug here to be available in catch blocks for error messages
    ...
    try {
        ...
        if (!audioRes.ok) {
            ...
            const fetchError: TranscriptionError = new Error(`MP3 fetch failed: ${audioRes.status}`);
        ...
        const transcript: string = await transcribe(tmpFile);
        ...
        } as ApiResponse<TranscriptionResponse>);
        ...
        // Clean up temp file if it exists
        if (tmpFile) {
            fs.unlink(tmpFile, (unlinkErr: NodeJS.ErrnoException | null): void => {
                if (unlinkErr) {
