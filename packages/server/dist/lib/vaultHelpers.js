 ...
 */
export async function createUserSecret(userId, tokenData) {
 ...
 */
export async function updateUserSecret(userId, tokenData) {
    const startTime = Date.now();
    try {
        const supabase = getSupabaseAdmin();
 ...
 */
export async function deleteUserSecret(userId, hardDelete = false, deletionReason = 'User request - GDPR Article 17') {
    const startTime = Date.now();
    const secretName = getSpotifySecretName(userId);
    try {
        const supabase = getSupabaseAdmin();
