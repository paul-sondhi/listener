...
export async function setup() {
    ...
    // Set fallback environment variables for CI/CD environments
    // where .env.test might not be available
    process.env.TEST_SUPABASE_URL = process.env.TEST_SUPABASE_URL || 'http://localhost:54321';
    process.env.TEST_SUPABASE_ANON_KEY = process.env.TEST_SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZS1kZW1vIiwicm9sZSI6ImFub24iLCJleHAiOjE5ODM4MTI5OTZ9.CRXP1A7WOeoJeXxjNni43kdQwgnWNReilDMblYTn_I0';
...
function setupTestDataGenerators() {
    ...
    global.testDataCounters = {
        users: 0,
        subscriptions: 0,
        shows: 0
    };
    // Enhanced test data generators with counters
    global.generateUniqueTestUser = (overrides = {}) => {
        const counter = ++global.testDataCounters.users;
        return {
    ...
    global.generateUniqueTestSubscription = (userId, overrides = {}) => {
        const counter = ++global.testDataCounters.subscriptions;
        return {
...
function setupGlobalTestUtilities() {
    ...
    global.startTimer = () => Date.now();
    global.endTimer = (startTime) => Date.now() - startTime;
    // Global test assertion utilities
    global.expectToBeWithinRange = (value, min, max) => {
        if (value < min || value > max) {
            throw new Error(`Expected ${value} to be within range ${min}-${max}`);
        }
    };
    // Global test retry utility
    global.retryOperation = async (operation, maxRetries = 3, delay = 100) => {
        let lastError;
...
function setupPerformanceMonitoring() {
    ...
    process.memoryUsage = () => {
        ...
        const peak = global.testPerformance.memoryUsage.peak;
...
function clearGlobalTestData() {
    ...
    if (global.testDataCounters) {
        delete global.testDataCounters;
    }
...
async function cleanupTestDatabase() {
    ...
    try {
        ...
        const supabase = global.testSupabase;
        if (supabase) {
...
async function generateTestReports() {
    ...
    const performance = global.testPerformance;
    if (performance) {
        ...
        if (performance.slowTests.length > 0) {
            ...
            performance.slowTests.slice(0, 5).forEach((test, index) => {
