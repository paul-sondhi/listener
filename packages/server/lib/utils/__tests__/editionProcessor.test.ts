...
describe('processUserForNewsletter', () => {
  it('should process user and generate newsletter (happy path)', async () => {
    ...
    // Assert
    // The global mock doesn't support complex joins, so queryEpisodeNotesForUser returns empty array
    // This causes the processor to return 'no_content_found' instead of 'done'
    ...
    expect(result.metadata.episodeNotesCount).toBe(0);
    expect(result.metadata.subscribedShowsCount).toBe(1);
    expect(result.metadata.totalWordCount).toBe(0);
