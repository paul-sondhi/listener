...
describe('buildSubject', () => {
  it('formats a Date object correctly', () => {
    ...
    expect(buildSubject(date)).toBe('July 8, 2025');
  });

  it('formats an ISO string correctly', () => {
    // Use UTC date string to avoid timezone issues
    expect(buildSubject('2025-07-08T00:00:00.000Z')).toBe('July 8, 2025');
  });

  it('handles different months and days', () => {
    expect(buildSubject('2025-12-25T00:00:00.000Z')).toBe('December 25, 2025');
    expect(buildSubject('2025-01-01T00:00:00.000Z')).toBe('January 1, 2025');
  });

  it('handles invalid date input gracefully', () => {
    // Should return 'Invalid Date' for invalid input
    expect(buildSubject('not-a-date')).toBe('Invalid Date');
  });
